	        		...
	        		<div class="card-body">
		        		<div class="row">
			        		<div class="col-md-4" style="text-align: -webkit-center">
			        			<button  class="rem_cart btn btn-sm btn-outline-danger deleteFromCart"   data-id="<?php echo $row['cid'] ?>"><i class="fa fa-trash"></i></button>
    ...
    <script>
		...
		$('#checkout').click(function(){
			if('<?php echo isset($_SESSION['login_user_id']) ?>' == 1){
				location.replace("index.php?page=checkout2")
			}else{
				uni_modal("Checkout","login.php?page=checkout2")
			}
		})

		
		$('.deleteFromCart').click(function(e){
			console.log('heyy delete cliked')
			console.log( $(this).attr('data-id'));
			start_load()
			$.ajax({
				url:'admin/ajax.php?action=delete_from_cart',
				method: 'POST',
